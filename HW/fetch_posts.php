<?php
    ...
    $query = "SELECT DISTINCT posts.id as id, users.username as user, nlikes, ncomments, url_image, description, time FROM posts JOIN users on posts.user = users.id LEFT JOIN friends ON posts.user = friends.user_friend WHERE posts.user IN (SELECT users.id FROM users WHERE username = '$user') OR friends.user IN (SELECT users.ID FROM users WHERE users.username = '$user') ORDER BY posts.id DESC;";
